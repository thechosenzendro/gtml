...
try:
    ...
    for import_declaration in code.findall('./imports/import'):
        ...
        if "from" in import_obj:
            import_statement = f'from {import_obj["from"]} import {import_obj["package"]}'
        else:
    ...
    print("An error occured during importing libraries. Did you close all the tags properly? Remember: <import/> not <import>!")
...
try:
    ...
    for meta in code.findall('./metadata/meta'):
        ...
        try:
            ...
            raise TypeError('Either key or value are not declared. Try checking your meta tags.') 
...
for attribute in important_window_attributes:
    if important_window_attributes[attribute] != True:
        raise TypeError(f'The property "{attribute}" was not defined and is set as important. Please define the property.')
...
for iteration, component in enumerate(code.findall('./window/ui//*')):
    ...
    parent_of_component_xpath = ''.join([item + '/' for item in parent_of_component_arr]).replace("/gtml", ".")[:-1]
    ...
    if parent_of_component.tag != 'ui':
        parent_of_component_id += mem_address_to_id_dict[parent_of_component_mem_address]
    else:
        parent_of_component_id += 'main_box'
    print(f'Processing component "{tag}" with id "{component_id}" and memory address "{mem_address}"...')
    component_code = ''
    if tag in components:
        #Adding innertext property to the component function (it is deleting the last 4 characters because the innertext() adds 4 spaces to the string.)
        ...
        component_definition = components[tag]
        ...
        component_definition = component_definition.replace("#parent_id", parent_of_component_id)
        
        #Adding id property to the component function
        component_definition = component_definition.replace("#innertext", f'f"""{innerText}"""'.replace(f'\n', ''))
        #Adding the parent property to the component function
        component_definition = component_definition.replace("#id", 'self.' + component_id)

        #And back to dictionary
        component_definition = ast.literal_eval(component_definition)
        ...
        for attribute in attributes:
            if not attribute in ignore_attribute:
                ...
                if attribute in attribute_list:
                    ...
                    #string means any. Checking if there are defined types.
                    if not '#string' in attribute_list[attribute]:
                        if attribute_value in attribute_list[attribute]:
                            ...
                            print(f' Attribute code:{attribute_code}')
                        else:
                            raise Exception(f'Value "{attribute_value}" of attribute "{attribute}" is not present in component definition. Possible values are: {list(attribute_list[attribute].keys())}')
                    else:
                        attribute_code = attribute_list[attribute]["#string"].replace("#string", attribute_value) 
                        attribute_code += f"\n        #Here are the attributes for {component_id}"
                        component_code = component_code.replace(f"#Here are the attributes for {component_id}", re.sub('^\s+', '', attribute_code))
                else:
                    raise Exception(f'The attribute "{attribute}" is not defined on the component "{tag}". Possible attributes are: {list(attribute_list.keys())}')

        #Adding the Replace tag to the component codes
        component_code += f'\n#Here are the components'
        buffer = buffer.replace("#Here are the components", component_code)
    else:
        raise Exception('Component not found in components dictionary. Check for type errors or declare a custom component.')
...
global_code = code.find('./code/global')
global_code = re.sub(r'\s+$', '', innertext(global_code))
